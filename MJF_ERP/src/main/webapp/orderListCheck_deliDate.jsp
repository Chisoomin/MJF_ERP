...
String sql = "SELECT *, COUNT(order_num) FROM order_table WHERE delivery_date between date('" + startDate
		+ "') and date('" + endDate + "') GROUP BY order_num HAVING COUNT(order_num) > 0;";
...
<script>
		function memberInfo(memberId) { window.open("MJF_Layout.jsp?pageChange=memberInfo.jsp?memberId=" + memberId, "memberInfo", "width=auto, height=auto, left=auto, top=auto"); }
		function accountInfo(accountCode) { window.open("MJF_Layout.jsp?pageChange=accountInfo.jsp?accountCode=" + accountCode, "accountInfo", "width=auto, height=auto, left=auto, top=auto"); }
		function productInfo(productCode) { window.open("MJF_Layout.jsp?pageChange=productInfo.jsp?productCode=" + productCode, "productInfo", "width=auto, height=auto, left=auto, top=auto"); }
</script>
...
function todayDateLookup() {
	location.replace("MJF_Layout.jsp?pageChange=orderListCheck_todayDate.jsp");
}

function allDateLookup() {
	location.replace("MJF_Layout.jsp?pageChange=orderListCheck.jsp");
}

function orderDateLookup() {
	location.replace("MJF_Layout.jsp?pageChange=orderListCheck_orderDate.jsp?startDate=" + startDate + "&endDate=" + endDate);
...
function deliDateLookup() {
	location.replace("MJF_Layout.jsp?pageChange=orderListCheck_deliDate.jsp?startDate=" + startDate + "&endDate=" + endDate);
// 	alert(document.getElementById('selectDate').value);
// 	alert("시작 날짜 : " + startDate);
// 	alert("종료 날짜 : " + endDate);
}
